...
type Quest = { id: number; label: string; xp: number; done: boolean };
const initialQuests: Quest[] = [
  { id: 1, label: "Morning movement", xp: 30, done: true },
  { id: 2, label: "Read for 20 minutes", xp: 20, done: true },
  { id: 3, label: "Deep work: Life Dashboard", xp: 50, done: false },
  { id: 4, label: "Call someone you love", xp: 25, done: false },
  { id: 5, label: "Plan tomorrow", xp: 15, done: false },
];
const attributes = [
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (saved) { try { setQuests(JSON.parse(saved)); } catch { /* use starter state */ } }
    setReady(true);
  }, []);
  ...
  const addQuest = () => {
    ...
    setQuests((items) => [...items, { id: Date.now(), label: label.trim(), xp: 20, done: false }]);
  };
  ...
  return <main className="min-h-screen overflow-hidden bg-[#070b18] text-[#f4f6ff]">
    ...
    <div className="relative mx-auto grid min-h-screen max-w-[1540px] lg:grid-cols-[244px_minmax(0,1fr)]">
      ...
      <section id="dashboard" className="w-full max-w-[1400px] px-5 py-8 sm:px-9 lg:px-15 lg:py-12">
        <header className="mb-9 flex items-start justify-between gap-5"><div><p className="mb-2 font-mono text-[10px] tracking-[.12em] text-[#8e98b9]">TUESDAY · SEPTEMBER 16</p><h1 className="text-3xl font-extrabold tracking-[-.06em] sm:text-[34px]">Good evening, Starlight <span className="text-[#d2c7ff]">✦</span></h1></div><div className="flex gap-2.5"><button aria-label="Notifications" className="grid size-10 place-items-center rounded-xl border border-white/10 bg-[#171d37a3] text-lg">♧</button><button className="hidden rounded-xl border border-white/10 bg-[#171d37a3] px-3 text-xs font-bold text-[#cbd2ec] sm:block"><span className="mr-1 text-[#9c88ff]">◎</span>Focus mode</button></div></header>
